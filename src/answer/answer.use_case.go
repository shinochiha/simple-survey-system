...
// UseCaseHandler provides a convenient interface for Answer use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Answer data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Answer, error) {
	res := Answer{}

	// check permission
	err := u.Ctx.ValidatePermission("answers.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Answer data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("answers.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.PaginationInfo(tx, &Answer{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Find(tx, &Answer{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Answer with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("answers.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Answer data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("answers.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Answer data for the specified ID with specified parameters.
...
// DeleteByID deletes the Answer data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("answers.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Answer data.
func (u *UseCaseHandler) setDefaultValue(old Answer) error {
	if !old.ID.Valid {
