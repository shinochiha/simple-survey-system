...
// UseCaseHandler provides a convenient interface for Choice use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Choice data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Choice, error) {
	res := Choice{}

	// check permission
	err := u.Ctx.ValidatePermission("choices.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Choice data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("choices.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.PaginationInfo(tx, &Choice{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Find(tx, &Choice{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Choice with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("choices.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Choice data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("choices.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Choice data for the specified ID with specified parameters.
...
// DeleteByID deletes the Choice data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("choices.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Choice data.
func (u *UseCaseHandler) setDefaultValue(old Choice) error {
	if !old.ID.Valid {
