...
// UseCaseHandler provides a convenient interface for Response use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Response data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Response, error) {
	res := Response{}

	// check permission
	err := u.Ctx.ValidatePermission("responses.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Response data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("responses.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.PaginationInfo(tx, &Response{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Find(tx, &Response{}, u.Query)
	if err != nil {
		return res, app.NewError(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Response with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("responses.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Response{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Response data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("responses.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Response data for the specified ID with specified parameters.
...
// DeleteByID deletes the Response data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("responses.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Response data.
func (u *UseCaseHandler) setDefaultValue(old Response) error {
	if !old.ID.Valid {
